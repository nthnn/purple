...
JsonValueType JsonValue::get_type() {
    return this->type;
}
...
JsonValue& JsonValue::operator[](
    ...
    if(!this->is_object()) {
        if(this->is_null()) {
            ...
            "Value is not an object, cannot access by key."
        );
    }
...
const JsonValue& JsonValue::operator[](
    const std::string& key
) const {
    if(!this->is_object())
        throw JsonParseException(
...
void JsonValue::reserve(size_t capacity) {
    if(this->is_array())
        std::get<JsonArray>(this->data)
            .reserve(capacity);
    else if(this->is_object())
        std::get<JsonObject>(this->data)
            .reserve(capacity);
}
...
void JsonValue::serialize_internal(
    std::ostream& os,
    bool pretty,
    int indent_level
) const {
    ...
    switch(this->type) {
        ...
        case JsonValueType::Object: {
            ...
            for(const auto& pair : obj) {
                ...
                pair.second.serialize_internal(
                    os,
                    pretty,
                    indent_level + 1
                );
...
JsonValue JsonParser::parse(const std::string& json_str) {
    ...
    if(this->pos < this->json_str.length())
        throw JsonParseException(
            "Unexpected characters after JSON root element."
        );
    return result;
}

void JsonParser::skip_whitespace() {
    while(this->pos < this->json_str.length() &&
...
char JsonParser::peek() const {
    return (this->pos >= this->json_str.length()) ?
        '\0' : this->json_str[this->pos];
}
...
JsonValue JsonParser::parse_number() {
    ...
    else while(std::isdigit(this->peek()))
        this->next();
    ...
    std::string_view num_view = this->json_str.substr(
        start_pos,
        this->pos - start_pos
    );
    ...
    while(this->peek() != '"') {
        ...
        if(c == '\\') {
            if(this->pos > start_segment)
                result.append(
                    this->json_str.data() + start_segment,
    ...
    switch(escape_char) {
        ...
        case 'u': {
            ...
            if(unicode_val >= 0x00 && unicode_val <= 0x7F)
                ...
                "Unsupported non-ASCII unicode escape sequence \\u" +
                    hex_str +
...
JsonValue JsonParser::parse_string() {
    if(!this->match('"'))
        throw JsonParseException(
            "Expected '\"' to start string at position " +
                std::to_string(this->pos)
        );
    ...
    if(!this->match('"'))
        throw JsonParseException(
            "Expected '\"' to end string at position " +
                std::to_string(this->pos)
        );
...
JsonValue JsonParser::parse_array() {
    if(!this->match('['))
        throw JsonParseException(
            "Expected '[' to start array at position " +
                std::to_string(this->pos)
        );
...
JsonValue JsonParser::parse_object() {
    if(!this->match('{'))
        throw JsonParseException(
            "Expected '{' to start object at position " +
                std::to_string(this->pos)
        );
    ...
    while(true) {
        ...
        if(!this->match('"'))
            throw JsonParseException(
                "Expected '\"' for object key at position " +
                    std::to_string(this->pos)
            );
        ...
        if(!this->match('"'))
            throw JsonParseException(
                "Expected '\"' to end object key at position " +
                    std::to_string(this->pos)
            );
