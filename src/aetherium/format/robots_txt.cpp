...
std::set<std::string> RobotsTxt::get_sitemaps() {
...
bool RobotsTxt::operator==(const RobotsTxt& other) const {
    if(this->user_agent_blocks.size() !=
            other.user_agent_blocks.size() ||
        this->sitemaps != other.sitemaps)
        return false;

    for(size_t i = 0; i < this->user_agent_blocks.size(); ++i)
        if(!(this->user_agent_blocks[i] ==
                other.user_agent_blocks[i]))
            return false;

    return true;
}

RobotsTxt RobotsTxt::parse(
    ...
    while(std::getline(ss, line)) {
        ...
        std::string directive = robots_str_trim(
            line.substr(0, colonPos)
        );
        std::string value = robots_str_trim(
        ...
        std::transform(
            directive.begin(),
            directive.end(),
            directive.begin(), ::tolower
        );

        if(directive == "user-agent") {
            if(in_user_agent_block) {
                robots.user_agent_blocks.push_back(
                    current_block
                );
...
bool RobotsTxt::is_path_allowed(
    ...
    std::sort(
        ...
        sorted_rules.end(),
        [](
            const RobotsTxtRule& a,
            const RobotsTxtRule& b
        ) {
    ...
    for(const auto& rule : sorted_rules)
        if(path.rfind(rule.path, 0) == 0) {
            ...
            std::string effective_rule_path = rule_ends_with_path ?
                rule.path.substr(0, rule.path.length() - 1) :
                rule.path;
            ...
            if(path.rfind(effective_rule_path, 0) == 0) {
                ...
                else path_matches_rule =
                    path.rfind(effective_rule_path, 0) == 0;
