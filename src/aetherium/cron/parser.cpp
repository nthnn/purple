...
std::set<int> CronParser::parse_field(
    ...
    while(std::getline(ss, item, ',')) {
        if(item == "*")
            ...
            if(base != "*") {
                ...
                if(dash_pos != std::string::npos) {
                    startVal = CronParser::conv_name_to_value(
                        base.substr(0, dash_pos),
                        parse_months,
                        parse_days_of_week
                    );

                    endVal = CronParser::conv_name_to_value(
                        base.substr(dash_pos + 1),
                        parse_months,
                        parse_days_of_week
                    );
                }
            ...
            int start = CronParser::conv_name_to_value(
                item.substr(0, dash_pos),
                parse_months,
                parse_days_of_week
            );
            int end = CronParser::conv_name_to_value(
                item.substr(dash_pos + 1),
                parse_months,
                parse_days_of_week
            );
            ...
            int val = CronParser::conv_name_to_value(
                item,
                parse_months,
                parse_days_of_week
            );
...
int CronParser::conv_name_to_value(
...
CronParsedFields CronParser::parse(
    ...
    try {
        fields.minutes = CronParser::parse_field(
            segments[0],
            0, 59
        );

        fields.hours = CronParser::parse_field(
            segments[1],
            0, 23
        );

        fields.days_of_month = CronParser::parse_field(
            segments[2],
            1, 31
        );

        fields.months = CronParser::parse_field(
            segments[3],
            1, 12,
            true
        );

        fields.days_of_week = CronParser::parse_field(
            segments[4],
            0, 7,
            false,
            true
