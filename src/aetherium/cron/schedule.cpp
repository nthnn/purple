...
TimePoint CronSchedule::get_next_runtime(TimePoint start_tm) const {
    std::time_t tt_start = std::chrono::system_clock
        ::to_time_t(start_tm);
    ...
    for(
        int iteration = 0;
        iteration < 365 * 24 * 60 * 2;
        ++iteration
    ) {
        std::time_t tt_current = std::chrono::system_clock
            ::to_time_t(current_check_time);
        std::tm tm_current = *std::gmtime(&tt_current);

        int current_dow = tm_current.tm_wday;
        if(this->parsed_fields.days_of_week.count(7) &&
            current_dow == 0)
            current_dow = 7;
        ...
        bool day_of_month_match = this->parsed_fields
            .days_of_month
            .count(tm_current.tm_mday);
        bool day_of_week_match = this->parsed_fields
            .days_of_week
            .count(current_dow);
        ...
        else day_match_criteria_met = day_of_month_match ||
            day_of_week_match;
        ...
        for(const std::string& job_id : jobs_to_exec) {
            this->task_manager.go([this, job_id]() {
                ...
                try {
                    ...
                    std::cerr << "Unknown error executing job '"
                        << job_id
                        << "'"
                        << std::endl;
                }
...
bool CronScheduler::set_job_enabled(
    const std::string& id,
    bool enabled
) {
    std::lock_guard<std::mutex> lock(this->jobs_mtx);
