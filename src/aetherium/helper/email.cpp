...
EmailAddress::EmailAddress()
    : display_name(""), address(""), local_part(""), domain(""), valid(false) {}

EmailAddress::EmailAddress(const std::string &email_addr)
    : display_name(""), address(""), local_part(""), domain(""), valid(false) {
  this->parse(email_addr);
}

void EmailAddress::parse(const std::string &email_addr) {
  std::string clean_str = email_str_trim(email_addr);
  size_t lt_pos = clean_str.find('<');
  size_t gt_pos = clean_str.find('>');

  if (lt_pos != std::string::npos && gt_pos != std::string::npos &&
      gt_pos > lt_pos) {
    this->display_name = email_str_trim(clean_str.substr(0, lt_pos));

    if (this->display_name.length() >= 2 && this->display_name.front() == '"' &&
        this->display_name.back() == '"')
      this->display_name =
          this->display_name.substr(1, this->display_name.length() - 2);

    this->address =
        email_str_trim(clean_str.substr(lt_pos + 1, gt_pos - (lt_pos + 1)));
  } else
    this->address = clean_str;
...
const std::string &EmailAddress::get_address() const { return this->address; }

const std::string &EmailAddress::get_local_part() const {
  return this->local_part;
}

const std::string &EmailAddress::get_domain() const { return this->domain; }

bool EmailAddress::is_valid() const {
  const std::regex email_regex(R"(^[^@\s]+@[^@\s]+\.[^@\s]+$)");

  return std::regex_match(this->address, email_regex);
}
...
EmailMessage::EmailMessage()
    : headers(), body(""), content_type("text/plain"), boundary("") {}

const std::map<std::string, std::string> &EmailMessage::get_headers() const {
  return this->headers;
}

std::string EmailMessage::get_header(const std::string &name) const {
...
const std::string &EmailMessage::get_body() const { return this->body; }

const std::string &EmailMessage::get_content_type() const {
  return this->content_type;
}

const std::string &EmailMessage::get_boundary() const { return this->boundary; }

void EmailMessage::set_headers(
    const std::map<std::string, std::string> &new_headers) {
  this->headers = new_headers;
}

void EmailMessage::set_header(const std::string &name,
...
void EmailMessage::set_content_type(const std::string &new_content_type) {
  this->content_type = new_content_type;
}

void EmailMessage::set_boundary(const std::string &new_boundary) {
  ...
  while (std::getline(iss, line)) {
    if (in_headers && email_str_trim(line).empty()) {
      if (!current_header_name.empty())
        email.set_header(current_header_name,
    ...
    if (in_headers) {
      if (!line.empty() && (line[0] == ' ' || line[0] == '\t')) {
        if (!current_header_name.empty())
          current_header_value += " " + email_str_trim(line);
      } else {
        if (!current_header_name.empty())
          email.set_header(current_header_name,
                           email_str_trim(current_header_value));
        ...
        if (colon_pos != std::string::npos) {
          current_header_name = email_str_trim(line.substr(0, colon_pos));
          current_header_value = email_str_trim(line.substr(colon_pos + 1));
        } else {
          if (!current_header_name.empty())
            current_header_value += " " + email_str_trim(line);
        }
      ...
      email.set_body(email.get_body() + line + "\n");
  }

  if (in_headers && !current_header_name.empty())
    email.set_header(current_header_name, email_str_trim(current_header_value));

  std::string content_type_header = email.get_header("Content-Type");
  if (!content_type_header.empty()) {
    size_t semicolon_pos = content_type_header.find(';');
    if (semicolon_pos != std::string::npos) {
      email.set_content_type(
          email_str_trim(content_type_header.substr(0, semicolon_pos)));
      ...
      while (std::getline(paramStream, param, ';')) {
        ...
        if (eq_pos != std::string::npos) {
          std::string key = email_str_trim(param.substr(0, eq_pos));
          std::string value = email_str_trim(param.substr(eq_pos + 1));
      ...
      email.set_content_type(email_str_trim(content_type_header));
  }
