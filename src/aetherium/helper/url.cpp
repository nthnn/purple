...
std::string UrlParser::get_original_url() const {
    return this->original_url;
}

std::string UrlParser::get_scheme() const {
    return this->scheme;
}

void UrlParser::set_scheme(const std::string& scheme) {
    this->scheme = scheme;
}

std::string UrlParser::get_host() const {
    return this->host;
}

void UrlParser::set_host(const std::string& host) {
    this->host = host;
}

std::string UrlParser::get_port() const {
    return this->port;
}

void UrlParser::set_port(const std::string& port) {
    this->port = port;
}
...
void UrlParser::set_path(const std::string& path) {
    this->path = path;
}
...
const std::map<
    std::string,
    std::string
>& UrlParser::get_query_params() const {
    return this->query_params;
}

std::string UrlParser::get_fragment() const {
...
std::string UrlParser::get_param(const std::string& key) const {
    ...
    return it != this->query_params.end() ?
        it->second : "";
}
...
void UrlParser::add_query_param(
...
bool UrlParser::remove_query_param(
    const std::string& key
) {
    return this->query_params.erase(key) > 0;
}

void UrlParser::clear_queries() {
    this->query_params.clear();
}

bool UrlParser::has_query_params() const {
    return !this->query_params.empty();
}
...
bool UrlParser::is_secure() const {
    ...
    std::transform(
        lower_scheme.begin(),
        lower_scheme.end(),lower_scheme.begin(),
        [](unsigned char c){
...
std::string UrlParser::get_file_name() const {
    ...
    if(last_slash == std::string::npos ||
        last_slash == this->path.length() - 1)
        return "";

    std::string file_name_with_extension = this->path.substr(
        last_slash + 1
    );
    ...
    if(q_pos != std::string::npos)
        file_name_with_extension =
            file_name_with_extension.substr(0, q_pos);

    size_t h_pos = file_name_with_extension.find('#');
    if(h_pos != std::string::npos)
        file_name_with_extension =
            file_name_with_extension.substr(0, h_pos);
...
void UrlParser::parse_url() {
    ...
    if(std::regex_match(this->original_url, matches, url_regex)) {
        ...
        if(this->path.empty() &&
            ...
            this->original_url.find("://") != std::string::npos &&
            this->original_url.find("://") + 3 ==
                this->original_url.length())
            this->path = "/";
        else if(this->path.empty() &&
            this->original_url.find("://") != std::string::npos)
            this->path = "/";
...
void UrlParser::parse_query_params(
