...
SocketCloser::operator int() const {
    return this->fd;
}
...
RequestHandler Weblet::load_response(
    int shared_mods,
    ...
    typedef Response (*DynamicHandlerPtr)(
        Request,
        std::map<std::string, std::string>
    );
    
    DynamicHandlerPtr func_ptr = (DynamicHandlerPtr) dlsym(
        handle,
        response_name.c_str()
    );
...
void Weblet::start() {
    Aetherium::Concurrent::go<std::function<void()>>(
        ...
        [this] {
            ...
            address.sin_addr.s_addr = (this->hostname == "localhost" ||
                this->hostname == "127.0.0.1") ?
                    INADDR_ANY : inet_addr(this->hostname.c_str());
...
bool Weblet::is_running() {
    return this->server_desc != -1;
}

ssize_t Weblet::safe_send(
    int sock_desc,
    ...
    while(total_sent < len) {
        ...
        if(bytes_sent <= 0) {
            ...
            else this->handler_exception("Connection failed safe send function failed");
...
ssize_t Weblet::safe_recv_to_vec(
    int sock_desc,
    ...
    while(total_received < len_to_read) {
        ssize_t bytes_received = recv(
            sock_desc,
            buffer.data() + start_pos + total_received,
            len_to_read - total_received,
            flags
        );
        ...
        if(bytes_received <= 0) {
            ...
            else this->handler_exception(
                "Safe receiving mechanism vector failed"
            );
...
void Weblet::parse_url_enc_data(
    ...
    auto url_decode = [exception_fn = this->handler_exception](
        ...
        for(size_t i = 0; i < encoded_str.length(); ++i) {
            if(encoded_str[i] == '+')
                ...
                i + 2 < encoded_str.length()) {
                try {
                    decoded_str += static_cast<char>(std::stoi(
                        encoded_str.substr(i + 1, 2),
                        nullptr,
                        16
                    ));
                    i += 2;
                    ...
                    exception_fn(
                        "Malformed URL encoding encountered: " +
                            encoded_str.substr(i, 3)
                    );
                    decoded_str += encoded_str[i];
                }
            }
            else decoded_str += encoded_str[i];
        }
...
void Weblet::parse_multipart_data(
    const std::string& body,
    ...
    while(true) {
        size_t part_start_pos = body_view.find(
            delimiter,
            current_pos
        );
        ...
        if(body_view.length() >= part_start_pos + 2 &&
            ...
            this->handler_exception(
                "Malformed multipart part: boundary not followed by CRLF; will be skipped"
            );
        ...
        size_t part_end_pos = body_view.find(
            delimiter,
            part_start_pos
        );
        ...
        if(part_end_pos == std::string::npos) {
            this->handler_exception(
                "Malformed multipart body: part without end delimiter; skipping remaining body"
            );
            break;
        }

        std::string_view part_content_view = body_view.substr(
            part_start_pos,
            part_end_pos - part_start_pos
        );
        ...
        std::string_view part_headers_view = part_content_view.substr(
            0, headers_end
        );
        std::string_view part_body_view = part_content_view.substr(
            headers_end + 4
        );

        if(part_body_view.length() >= 2 &&
            part_body_view.substr(part_body_view.length() - 2) == "\r\n"
        ) part_body_view = part_body_view.substr(
            0,
            part_body_view.length() - 2
        );
        ...
        if(part_headers.count("Content-Disposition")) {
            ...
            if(std::regex_search(disposition_str, match_name, name_regex)) {
                ...
                this->handler_exception(
                    "Multipart part Content-Disposition without 'name' attribute; skipping part"
                );
            ...
            else request.form_fields[field_name_str] = std::string(part_body_view);
        }
        else this->handler_exception(
...
std::string Weblet::build_response_str(const Response& response) {
    ...
    for(const auto& header : response.headers)
        response_stream << header.first
            << ": "
            << header.second
            << "\r\n";

    for(const auto& cookie : response.cookies)
        response_stream << "Set-Cookie: "
            << cookie.second
            << "\r\n";
...
void Weblet::handle_client(int client_socket_fd) {
    ...
    while(header_end_pos == std::string::npos &&
        ...
        ssize_t bytes_read_chunk = recv(
            client_socket_fd,
            raw_request_bytes.data() + total_received_bytes,
            4096,
            0
        );
        ...
        if(bytes_read_chunk <= 0) {
            ...
            this->handler_exception(
                "Connection closed or error during header read after " +
                    std::to_string(total_received_bytes) +
                    " bytes"
            );
            break;
        }
        ...
        std::string_view current_data_view(
            raw_request_bytes.data(),
    ...
    if(header_end_pos == std::string::npos) {
        ...
        this->safe_send(
            client_socket_fd,
            build_response_str(response)
        );
        return;
    }

    std::string_view full_request_view(
        raw_request_bytes.data(),
        total_received_bytes
    );
    std::string_view request_body_initial_view = full_request_view.substr(
        header_end_pos + 4
    );
    std::string request_headers_string(full_request_view.substr(
        0, header_end_pos
    )); 
    ...
    if(request.headers.count("Content-Length"))
        try {
            ...
            response = handle_error(400, "Bad Request: Invalid Content-Length header.");
            this->safe_send(
                client_socket_fd,
                this->build_response_str(response)
            );
    ...
    std::string request_body_str(
        request_body_initial_view
    );
    size_t body_already_read =
        request_body_initial_view.length();

    if(static_cast<size_t>(content_length) >
        body_already_read) {
        size_t remaining_bytes_to_read = content_length - body_already_read;
        raw_request_bytes.reserve(
            total_received_bytes + remaining_bytes_to_read
        );
        ...
        if(read_result < (ssize_t) remaining_bytes_to_read) { 
            ...
            this->handler_exception(
                "Connection closed during body read; expected " +
                    std::to_string(remaining_bytes_to_read) + ", got " +
                    std::to_string(read_result) + " more bytes"
            );

            response = this->handle_error(
                400,
                "Bad Request: Incomplete request body."
            );
    ...
    if(request.headers.count("Content-Type")) {
        ...
        else if(content_type.rfind(
            "application/x-www-form-urlencoded", 0
        ) == 0) {
            request.contents = request_body_str;
            this->parse_url_enc_data(
                request_body_str,
                request
            );
        }
        else request.contents = request_body_str;
    }
    else request.contents = request_body_str;

    this->safe_send(
        client_socket_fd,
        this->build_response_str(
            this->route_request(request)
        )
    );
}
...
Response Weblet::route_request(const Request& request) {
    ...
    if(!this->public_dir.empty()) {
        ...
        if(std::filesystem::exists(filepath) &&
            ...
            else filename_part = requested_path;

            std::string spa_index_path = this->public_dir +
                "/index.html";
            bool is_asset_request = (
                filename_part.find('.') != std::string::npos
            );

            if(!is_asset_request &&
                std::filesystem::exists(spa_index_path) &&
...
Response Weblet::handle_error(
    ...
    if(this->error_handlers.count(error_code)) {
        ...
        if(file.is_open()) {
            ...
            response.contents = "<h1>" +
                std::to_string(error_code) +
                " - Error</h1><p>Failed to load error page: " +
                errorfilepath + "</p>";
