...
const std::vector<CardValidator::CardRule> CardValidator::cardRules = {
    ...
    CardValidator::CardRule(CardProvider::MASTERCARD,
                            {"51",   "52",   "53",   "54",   "55",   "2221",
                             "2222", "2223", "2224", "2225", "2226", "2227",
                             "2228", "2229", "223",  "224",  "225",  "226",
                             "227",  "228",  "229",  "23",   "24",   "25",
                             "26",   "270",  "271",  "2720"},
                            {16}, 3),

    CardValidator::CardRule(CardProvider::AMERICAN_EXPRESS, {"34", "37"}, {15},
                            4),
    ...
    CardValidator::CardRule(
        ...
        {"300", "301", "302", "303", "304", "305", "36", "38", "39"}, {14}, 3),

    CardValidator::CardRule(
        CardProvider::JCB,
        {"3528", "3529", "353", "354", "355", "356", "357", "358"}, {16}, 3),

    CardValidator::CardRule(CardProvider::UNIONPAY, {"62"}, {16, 17, 18, 19},
                            3)};

CardInfo CardValidator::validate_card_num(const std::string &card_num) {
  CardInfo info;
  std::string clean_card_num = CardValidator ::remove_nondigits(card_num);

  info.card_num = clean_card_num;
  info.length = clean_card_num.length();

  if (clean_card_num.empty()) {
    info.validation_status = ValidationStatus::EMPTY_CARD_NUMBER;
    return info;
  }

  if (!CardValidator::is_all_digits(clean_card_num)) {
    info.validation_status = ValidationStatus::INVALID_CHARACTERS;
    return info;
  }

  info.is_valid_luhn = CardValidator::is_luhn_valid(clean_card_num);
  if (!info.is_valid_luhn) {
    info.validation_status = ValidationStatus::INVALID_LUHN;
    return info;
  }

  info.provider = CardValidator::detect_provider(clean_card_num);

  if (info.provider == CardProvider::UNKNOWN) {
    info.validation_status = ValidationStatus::INVALID_PREFIX;
    return info;
  }
  ...
  if (!len_matches) {
    info.validation_status = ValidationStatus::INVALID_LENGTH;
    return info;
  }
...
bool CardValidator::is_luhn_valid(const std::string &card_num) {
  std::string clean_card_num = CardValidator ::remove_nondigits(card_num);
...
CardProvider CardValidator::detect_provider(const std::string &card_num) {
  std::string clean_card_num = CardValidator ::remove_nondigits(card_num);
...
std::string CardValidator::get_provider_name(CardProvider provider) {
  switch (provider) {
...
ValidationStatus CardValidator::validate_expiry_date(int month, int year) {
  if (month < 1 || month > 12)
...
ValidationStatus
CardValidator::validate_expiry_date(const std::string &mm_yy_format) {
  int month, year;

  return !CardValidator::parse_expiry_date(mm_yy_format, month, year)
             ? ValidationStatus::INVALID_EXPIRY_FORMAT
             : CardValidator::validate_expiry_date(month, year);
}

ValidationStatus CardValidator::validate_cvcv_format(const std::string &cvv,
  ...
  if (expected_cvv_len == 0 && provider != CardProvider::UNKNOWN)
  ...
  return static_cast<int>(cvv.length()) != static_cast<int>(expected_cvv_len)
             ? ValidationStatus::INVALID_CVV_LENGTH
             : ValidationStatus::VALID;
}

std::string CardValidator::mask_card_num(const std::string &card_num,
                                         char mask_char, int unmasked_digits) {
  std::string clean_card_num = CardValidator ::remove_nondigits(card_num);

  if (clean_card_num.length() <= static_cast<std::size_t>(unmasked_digits))
...
std::string CardValidator::format_card_num(const std::string &card_num,
                                           char separator) {
  std::string formatted_num = "";
  std::string clean_card_num = CardValidator ::remove_nondigits(card_num);

  CardProvider provider = CardValidator ::detect_provider(clean_card_num);
  ...
  if (provider == CardProvider::AMERICAN_EXPRESS) {
    if (clean_card_num.length() > 4) {
      ...
      formatted_num = clean_card_num;
  } else
    for (size_t i = 0; i < clean_card_num.length(); ++i) {
      if (i > 0 && i % 4 == 0)
        formatted_num += separator;
...
std::string CardValidator::remove_nondigits(const std::string &str) {
...
bool CardValidator::is_all_digits(const std::string &str) {
  return std::all_of(str.begin(), str.end(), ::isdigit);
}

bool CardValidator::parse_expiry_date(const std::string &mm_yy_format,
                                      int &month, int &year) {
  std::string clean_fmt = CardValidator ::remove_nondigits(mm_yy_format);
