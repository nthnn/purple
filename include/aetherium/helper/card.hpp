...
class CardValidator {
public:
    static CardInfo validate_card_num(
        const std::string& card_num
    );

    static bool is_luhn_valid(
        const std::string& card_num
    );

    static std::string get_provider_name(
        CardProvider provider
    );

    static CardProvider detect_provider(
        const std::string& card_num
    );

    static ValidationStatus validate_expiry_date(
        int month,
        int year
    );
    ...
    static ValidationStatus validate_cvcv_format(
        const std::string& cvv,
        CardProvider provider
    );

    static std::string mask_card_num(
        const std::string& card_num,
        char mask_char = 'X',
        int unmasked_digits = 4
    );

    static std::string format_card_num(
        const std::string& card_num,
        char separator = ' '
    );

private:
    static CardProvider getProviderFromPrefixAndLength(
        const std::string& prefix,
        int length
    );

    static std::string remove_nondigits(
        const std::string& str
    );

    static bool is_all_digits(const std::string& str);
    static bool parse_expiry_date(
        const std::string& mm_yy_format,
        int& month,
        int& year
    );
