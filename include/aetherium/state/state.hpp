...
private:
    ...
    std::map<StateEnum, std::shared_ptr<
        State<StateEnum>>
    > child_state;
...
public:
    explicit State(
        ...
        name(name.empty() ?
            std::to_string(static_cast<int>(id)) : name
        ),
        entry_action(),
        exit_action(),
        child_state() { }
    ...
    void add_child_state(
        std::shared_ptr<State<StateEnum>> child_state
    ) {
        if(this->child_state.find(child_state->get_id())
            == this->child_state.end())
            this->child_state[child_state->get_id()] =
                std::move(child_state);
    }

    std::shared_ptr<State<StateEnum>> get_child_state(
        StateEnum child_id
    ) const {
        auto it = this->child_state.find(child_id);
        return it != this->child_state.end() ?
            it->second : nullptr;
    }

    bool contains(
        const State<StateEnum>& other_state
    ) const {
        return this->child_state.count(
            other_state.get_id()
        ) > 0;
    }
};
...
private:
    ...
    std::map<
        StateEnum,
        std::shared_ptr<State<StateEnum>>
    > states;
    std::map<
        EventEnum,
...
public:
    explicit StateMachine(
        const std::string& name = "<anon-state-mach>"
    ) : name(name),
        states(),
        events(),
        transitions(),
    ...
    std::shared_ptr<State<StateEnum>> add_state(
        StateEnum id,
        const std::string& name = ""
    ) {
        ...
        auto new_state = std::make_shared<
            State<StateEnum>
        >(id, name);
    ...
    std::shared_ptr<State<StateEnum>> get_state(
        StateEnum id
    ) const {
    ...
    std::shared_ptr<StateEvent<EventEnum>> add_event(
        EventEnum id,
        const std::string& name = ""
    ) {
        std::lock_guard<std::mutex> lock(this->mtx);
        ...
        auto new_event = std::make_shared<StateEvent<EventEnum>>(
            id,
            name
        );
    ...
    std::shared_ptr<StateEvent<EventEnum>> get_event(
        EventEnum id
    ) const {
    ...
    std::shared_ptr<
        Transition<StateEnum, EventEnum>
    > add_transition(
        StateEnum from_state_id,
        StateEnum to_state_id,
        EventEnum event_id
    ) {
        std::lock_guard<std::mutex> lock(this->mtx);
        ...
        auto new_transition = std::make_shared<
            Transition<StateEnum, EventEnum>
    ...
    bool process_event(EventEnum event_id) {
        ...
        std::shared_ptr<
            Transition<StateEnum, EventEnum>
        > chosen_transition = nullptr;

        if(this->transitions.count(this->current_state->get_id())) {
            const auto& state_transitions = this->transitions[
                this->current_state->get_id()
            ];

            if(state_transitions.count(event->get_id()))
                chosen_transition = state_transitions.at(
                    event->get_id()
                );
        }

        if(!chosen_transition &&
            this->global_transitions.count(event->get_id()))
            chosen_transition = this->global_transitions.at(
                event->get_id()
            );
    ...
    > add_global_transition(
        ...
        if(this->global_transitions.count(event_id))
            throw StateConfigurationException(
                "Global transition on event " +
                    event->get_name() +
                    " already exists"
            );

        auto dummy_from_state = this->states.begin()->second;
        auto new_global_transition = std::make_shared<
            Transition<StateEnum, EventEnum>
        >(dummy_from_state, to_state, event);
    ...
    > get_transitions_from_state(StateEnum from_state_id) const {
        ...
        if(!this->states.count(from_state_id))
            throw UnknownStateException(
                "State " +
                std::to_string(static_cast<int>(from_state_id)) +
                    " not found"
            );

        auto it = this->transitions.find(from_state_id);
        return it != this->transitions.end()
            ? it->second : ({});
    }
    ...
    void clear() {
        ...
        if(this->current_state != nullptr)
            throw std::logic_error(
                "Cannot clear state machine while it is running"
            );
