...
struct RobotsTxtRule {
  ...
  bool operator==(const RobotsTxtRule &other) const {
...
struct UserAgentBlock {
  ...
  UserAgentBlock();
  bool operator==(const UserAgentBlock &other) const;
};
...
public:
  ...
  void set_user_agent_blocks(std::vector<UserAgentBlock> blocks);
