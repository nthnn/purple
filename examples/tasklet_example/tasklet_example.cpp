...
int main() {
    ...
    std::cout << "-- C++ Tasklet and Channel Imitation --"
        << std::endl;

    TaskletManager manager(4);

    std::cout << "\n--- Example 1: Basic Tasklet ---"
        << std::endl;

    go(&manager, [] {
        std::cout << "Hello from goroutine 1!"
            << std::endl;
    });

    go<std::function<void()>>(&manager, [] {
        std::cout << "Hello from goroutine 2!"
            << std::endl;
    });

    std::cout << "\n--- Example 2: Tasklet with Panic ---"
        << std::endl;
    go(&manager, [] {
        std::cout << "Tasklet about to panic..."
            << std::endl;
    ...
    go(&manager, [&] {
        std::cout << "Sender (unbuffered, closed): Sending 10"
            << std::endl;
        unbuffered_ch_closed.send(10);

        std::cout << "Sender (unbuffered, closed): Sent 10"
            << std::endl;
        std::cout << "Sender (unbuffered, closed): Sending 20"
            << std::endl;
        unbuffered_ch_closed.send(20);

        std::cout << "Sender (unbuffered, closed): Sent 20"
            << std::endl;
        std::cout << "Sender (unbuffered, closed): Closing channel"
            << std::endl;
    ...
    go(&manager, [&] {
        ...
        if(result1.second)
            std::cout << "Receiver (unbuffered, closed): Received "
                << result1.first
                << std::endl;
        else std::cout << "Receiver (unbuffered, closed): "
    ...
    std::cout << "\n--- Example 4: Buffered Channel with Close ---"
        << std::endl;

    Channel<std::string> buffered_ch_closed(2);
    go(&manager, [&] {
        std::cout << "Sender (buffered, closed): Sending 'apple'"
            << std::endl;
        buffered_ch_closed.send("apple");

        std::cout << "Sender (buffered, closed): Sending 'banana'"
            << std::endl;
        buffered_ch_closed.send("banana");

        std::cout << "Sender (buffered, closed): Closing channel"
            << std::endl;
    ...
    go(&manager, [&] {
        ...
        if(ok1) 
            std::cout << "Receiver (buffered, closed): "
                ...
                << std::endl;
        else std::cout << "Receiver (buffered, closed): "
    ...
    go(&manager, [&] {
        std::cout << "Try-Sender (closed): Attempting to send 1.1"
            << std::endl;

        if(try_ch_closed.try_send(1.1))
            std::cout << "Try-Sender (closed): Sent 1.1 successfully."
                << std::endl;
        else std::cout << "Try-Sender (closed): "
                "Failed to send 1.1 (channel full/no receiver/closed)."
            << std::endl;
        try_ch_closed.close();

        std::cout << "Try-Sender (closed): Channel closed." << std::endl;
        std::cout << "Try-Sender (closed): Attempting to send 2.2 after close (will fail)" << std::endl;

        if(try_ch_closed.try_send(2.2))
            std::cout << "Try-Sender (closed): Sent 2.2 successfully."
                << std::endl;
        else std::cout << "Try-Sender (closed): "
    ...
    go(&manager, [&] {
        ...
        double val;
        std::cout << "Try-Receiver (closed): Attempting to receive"
            << std::endl;

        if(try_ch_closed.try_receive(val))
            std::cout << "Try-Receiver (closed): Received "
                << val
                << " successfully."
                << std::endl;
        else std::cout << "Try-Receiver (closed): "
