...
void print_email_addr(EmailAddress address) {
  ...
  std::cout << "  Display Name: "
            << (address.get_display_name().empty() ? "(None)"
                                                   : address.get_display_name())
            << std::endl;

  std::cout << "  Address: "
            << (address.get_address().empty() ? "(None)"
                                              : address.get_address())
            << std::endl;

  std::cout << "  Local Part: "
            << (address.get_local_part().empty() ? "(None)"
                                                 : address.get_local_part())
            << std::endl;

  std::cout << "  Domain: "
            << (address.get_domain().empty() ? "(None)" : address.get_domain())
            << std::endl;

  std::cout << "  Valid: " << (address.is_valid() ? "Yes" : "No") << std::endl;
}

void print_email_message(EmailMessage message) {
  std::cout << "--- Email Headers ---" << std::endl;
  for (const auto &pair : message.get_headers())
    std::cout << pair.first << ": " << pair.second << std::endl;

  std::cout << "---------------------" << std::endl;
  std::cout << "Content-Type: " << message.get_content_type() << std::endl;

  if (!message.get_boundary().empty())
    std::cout << "Boundary: " << message.get_boundary() << std::endl;
...
int main() {
  std::string raw_email1 = "From: John Doe <john.doe@example.com>\r\n"
  ...
  std::string raw_email2 =
      ...
      "boundary=\"----=_NextPart_000_0001_01D1A2B3.C4D5E6F7\"\r\n"
      "\r\n"
  ...
  EmailParser parser;
  std::cout << "Parsing Email 1:" << std::endl;
  ...
  std::cout << "\nEmail 1 Subject: " << email1.get_header("Subject")
            << std::endl;
  std::cout << "Email 2 Content-Type: " << email2.get_header("Content-Type")
            << std::endl;
  std::cout << "Email 2 Boundary (extracted): " << email2.get_boundary()
            << std::endl;

  EmailMessage new_email;
  new_email.set_header("From", "New Sender <new.sender@example.com>");
  new_email.set_header("To", "Recipient <recipient@example.com>");
  new_email.set_header("Subject", "This is a new email built from scratch!");
  ...
  std::cout << "\n--- Building a New Email ---" << std::endl;

  std::string built_email = new_email.build();
  std::cout << built_email << std::endl;
  std::cout << "----------------------------" << std::endl;

  std::cout << "\n--- Parsing the Built Email ---" << std::endl;

  EmailMessage parsedbuilt_email = parser.parse(built_email);
  print_email_message(parsedbuilt_email);
  std::cout << "\n--- Demonstrating EmailAddress Class ---" << std::endl;

  EmailAddress addr1("John Doe <john.doe@example.com>");
  std::cout << "Parsing 'John Doe <john.doe@example.com>':" << std::endl;
  print_email_addr(addr1);

  EmailAddress addr2("jane.smith@example.org");
  std::cout << "\nParsing 'jane.smith@example.org':" << std::endl;
  print_email_addr(addr2);

  EmailAddress addr3("\"Another User, Esq.\" <another.user@sub.domain.co.uk>");
  ...
  std::cout << "\nParsing 'invalid-email':" << std::endl;
  print_email_addr(addr4);

  EmailAddress addr5("test@localhost");

  std::cout << "\nParsing 'test@localhost':" << std::endl;
  ...
  std::cout << "Reconstructed: " << addr6.to_string() << std::endl;
