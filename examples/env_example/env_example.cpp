...
int main() {
  ...
  if (test_env_file.is_open()) {
    ...
    std::cout << "Created a dummy .env file for testing." << std::endl
              << std::endl;
  } else {
    std::cerr << "Error: Could not create dummy .env file." << std::endl;
    return 1;
  }
  ...
  if (env.load(".env")) {
    ...
    try {
      std::cout << "APP_NAME: " << env.get("APP_NAME") << std::endl;

      std::cout << "DB_HOST: " << env.get("DB_HOST") << std::endl;

      std::cout << "DB_PORT: " << env.get("DB_PORT") << std::endl;

      std::cout << "API_KEY: " << env.get("API_KEY") << std::endl;

      std::cout << "DEBUG: " << env.get("DEBUG") << std::endl;

      std::cout << "EMPTY_VAR: '" << env.get("EMPTY_VAR") << "'" << std::endl;

      std::cout << "MULTI_LINE_STRING: '" << env.get("MULTI_LINE_STRING") << "'"
                << std::endl;

      std::cout << "SINGLE_QUOTED: '" << env.get("SINGLE_QUOTED") << "'"
                << std::endl;

      std::cout << "SPACED_KEY: '" << env.get("SPACED_KEY") << "'" << std::endl;

      std::cout << "NON_EXISTENT_VAR (with default): "
                << env.get("NON_EXISTENT_VAR", "default_value") << std::endl;
    } catch (const std::out_of_range &e) {
      std::cerr << "Error accessing variable: " << e.what() << std::endl;
    }
  } else {
  ...
  std::remove(".env");
  std::cout << "\nCleaned up dummy .env file." << std::endl;
