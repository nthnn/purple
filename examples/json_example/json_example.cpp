...
int main() {
    ...
    try {
        ...
        std::cout << "Name: "
            << json_doc["name"].get_string()
            << std::endl;

        std::cout << "Age: "
            << static_cast<int>(json_doc["age"].get_number())
            << std::endl;
        ...
        std::cout << "GPA: "
            << json_doc["gpa"].get_number()
            << std::endl;

        std::cout << "Grades is null: "
            << (json_doc["grades"].is_null() ? "true" : "false")
            << std::endl;
        ...
        std::cout << "Address Street: "
            << json_doc["address"]["street"].get_string()
            << std::endl;

        std::cout << "Address City: "
            << json_doc["address"]["city"].get_string()
            << std::endl;

        std::cout << "Empty Array is array: "
            << json_doc["empty_array"].is_array()
            << std::endl;

        std::cout << "Empty Object is object: "
            << json_doc["empty_object"].is_object()
            << std::endl;

        std::cout << "Escaped string: "
            << json_doc["escaped_string"].get_string()
            << std::endl;
        ...
        int first_item_id = static_cast<int>(
            complex_doc["data"]["items"][0]["id"].get_number()
        );
    ...
    for(size_t i = 0; i < invalid_jsons.size(); ++i)
        try {
            ...
            std::cerr << "  Caught unexpected error type: "
                << e.what()
                << std::endl;
        }
