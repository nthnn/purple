...
int main() {
    ...
    for(const auto& card : test_cards) {
        CardInfo info = CardValidator
            ::validate_card_num(card);
        ...
        std::cout << "  Provider: "
            << CardValidator
                ::get_provider_name(info.provider)
            << std::endl;
        std::cout << "  Luhn Valid: "
            << (info.is_valid_luhn ? "Yes" : "No")
            << std::endl;
        ...
        std::cout << std::endl;
        std::cout << "  Masked: "
            << CardValidator::mask_card_num(card, '*', 4)
            << std::endl;
        std::cout << "  Formatted: "
            << CardValidator::format_card_num(card, '-')
            << std::endl;
    ...
    for(const auto& expiry : test_expirations) {
        ValidationStatus status = CardValidator
