...
Sitemap: https://www.example.com/another-sitemap.xml
    ...
    RobotsTxt parsed_robots_1 = RobotsTxt::parse(
        robots_txt_content_1
    );
    std::cout << "--- Parsing robots.txt content ---"
        << std::endl;
    ...
    std::cout << "\n--- Building robots.txt from parsed data ---"
        << std::endl;
    std::cout << built_robots
        << std::endl;

    std::cout << "\n--- Path Allowance Checks (Googlebot) ---"
        << std::endl;
    std::cout << "/private/ (Googlebot): "
        << (parsed_robots_1.is_path_allowed(
                "Googlebot",
                "/private/"
            ) ? "Allowed" : "Disallowed")
        << std::endl;
...
Allow: /path
    ...
    RobotsTxt parsed_robots_2 = RobotsTxt::parse(
        robots_txt_content_2
    );

    std::cout << "\n--- Path Allowance Checks (TestBot with specific rules) ---"
        << std::endl;
    std::cout << "/folder/file.html (TestBot): "
        << (parsed_robots_2.is_path_allowed(
                "TestBot",
                "/folder/file.html"
            ) ? "Allowed" : "Disallowed")
        << std::endl;
...
Disallow: /
    ...
    RobotsTxt parsed_robots_3 = RobotsTxt::parse(
        robots_txt_content_3
    );

    std::cout << "\n--- Path Allowance Checks (EvilBot - Disallow: /) ---"
        << std::endl;
    std::cout << "/ (EvilBot): "
        << (parsed_robots_3.is_path_allowed(
                "EvilBot",
                "/"
            ) ? "Allowed" : "Disallowed")
        << std::endl;
