...
Response handshake(
    ...
    if(request.cookies.count("sessionId"))
        std::cout << "  Handshake handler: Received sessionId cookie: "
            ...
            << std::endl;
    else std::cout << "  Handshake handler: No sessionId cookie received."
        << std::endl;
    ...
    std::cout << "  Handshake handler executed."
        << std::endl;
    return response;
}

Response fetch_employee(
    Request request,
    std::map<std::string, std::string> parameters
) {
    Response response;
    ...
    if(parameters.find("id") == parameters.end()) {
        ...
        std::cout << "  Fetch Employee handler: Missing ID."
            << std::endl;
        return response;
    }
    ...
    std::cout << "  Fetch Employee handler executed for ID: " << employee_id << std::endl;
    response.set_cookie(
        "lastViewedEmployee",
        employee_id,
        {{"Max-Age", "600"}}
    ...
    for(const auto& pair : request.upload_files) {
        ...
        if(outfile.is_open()) {
            ...
            ss << "\"original_filename\": \""
                << file.filename
                << "\", ";
            ss << "\"error\": \"Failed to save file to '"
                << save_path
                << "'\"";
            ss << "}";
        }
...
void create_public_files() {
    ...
    else std::cerr << "Warning: Could not create public/not-found.html"
        << std::endl;
    ...
    if(index_file.is_open()) {
        index_file << "<!DOCTYPE html>"
            ...
            "    <p>Try these paths:</p>"
            "    <ul>"
            "        <li><a href='/api/handshake'><code>/api/handshake</code></a></li>"
            "        <li><a href='/api/employee/101'><code>/api/employee/101</code></a> (Built-in Handler)</li>"
            "        <li><a href='/api/dynamic-employee/202'><code>/api/dynamic-employee/202</code></a> (Dynamic Module Handler)</li>"
            "        <li><a href='/upload.html'><code>/upload.html</code></a> (File Upload Example)</li>"
            "        <li><a href='/nonexistent.html'><code>/nonexistent.html</code></a> (to see 404)</li>"
            "    </ul>"
            "</body>"
            "</html>";
        index_file.close();
    } else std::cerr << "Warning: Could not create public/index.html"
        << std::endl;

    std::ofstream upload_file_html("public/upload.html");
    if(upload_file_html.is_open()) {
        upload_file_html << "<!DOCTYPE html>"
            "<head>"
            "    <meta charset=\"UTF-8\" />"
            "    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\" />"
    ...
    else std::cerr << "Warning: Could not create public/upload.html"
        << std::endl;
}
...
void delete_public_files() {
    ...
    if(std::filesystem::exists("uploads")) {
        ...
        std::cout << "Cleaned up 'uploads' directory."
            << std::endl;
    }
}
...
int main() {
    ...
    if(employee_mod_id != 0) {
        server.handle(
            "/api/dynamic-employee",
            server.load_response(
                employee_mod_id,
                "employee_fetch"
            )
        );
    ...
    else std::cerr << "Could not load dynamic employee module."
        << std::endl;
