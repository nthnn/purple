...
employee_fetch(DotEnv config, Request request,
  ...
  if (employee_id.empty()) {
    ...
    response.contents = "{\"message\": \"No employee ID provided.\", "
                        "\"example_url\": \"/api/dynamic-employee/123\"}";

    std::cout << "  Dynamic Employee handler: No ID provided" << std::endl;
  } else {
    response.contents = std::string("{\"employee_id\": \"") + employee_id +
                        "\", \"name\": \"Dynamic John Doe\", "
                        "\"position\": \"Dynamic Software Engineer\"}";
    response.status_code = 200;

    std::cout << "  Dynamic Employee handler executed for ID: " << employee_id
              << std::endl;
  }
